...
⚡ Flash Install - Compact Progress
${colors.reset}`);
...
const monitorInterval = setInterval(() => {
  try {
    ...
    if (currentCount > lastReportedCount || (Date.now() - lastReportTime) > 1000) {
      ...
      // Show progress - keep it short and sweet
      let statusText = `${colors.cyan}→ Packages: ${colors.bold}${currentCount}${colors.reset}`;
      
      // Add speed
      statusText += `${colors.cyan} (${packagesPerSecond.toFixed(1)} pkg/sec)${colors.reset}`;
      
      // Add last package if available
      if (lastNewPackage) {
        statusText += ` - Last: ${colors.green}${lastNewPackage}${colors.reset}`;
      }
